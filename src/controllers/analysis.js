...
// // src/controllers/analysis.js - processImagePatches function with improved error handling
...
exports.processImagePatches = async (req, res, next) => {
  try {
    ...
    // After sending the response, process the image
    try {
      // Process the image
      logger.info(`Starting image processing for image ${imageId}`);
      const patches = await processImage(imageId);
...
exports.runCnnAnalysis = async (req, res, next) => {
  try {
    ...
    runInitialAnalysis(imageId)
      .then(analysis => {
        logger.info(`Completed CNN analysis for image ${imageId} (ID: ${analysis.id})`);
      })
      .catch(error => {
...
exports.sendToYoloAnalysis = async (req, res, next) => {
  try {
    ...
    sendToYoloApi(imageId, initialAnalysisId)
      .then(analysis => {
        logger.info(`Completed YOLO analysis for image ${imageId} (ID: ${analysis.id})`);
      })
      .catch(error => {
        logger.error(`Failed to send image ${imageId} to YOLO API:`, error);
