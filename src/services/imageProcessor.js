...
const sharp = require('sharp');
const { SampleImage, ImagePatch } = require('../models');
const processThinSmear = require('./thinSmearProcessor');
const processThickSmear = require('./thickSmearProcessor');
...
function ensureDirectories() {
  const dirs = [
    UPLOADS_DIR,
    path.join(UPLOADS_DIR, 'original'),
    THUMBNAILS_DIR,
    PATCHES_DIR,
    path.join(UPLOADS_DIR, 'visualizations')
  ];
 ...
 * Process a sample image based on its type (thin or thick smear)
 * @param {string} imageId - The SampleImage ID
 * @returns {Promise<Array>} The extracted patch data
 */
async function processImage(imageId) {
  try {
    // Get image data from database
    ...
    logger.info(`Starting image processing for ${imageId}, type: ${imageData.imageType}`);
